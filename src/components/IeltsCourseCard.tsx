...
export default function IeltsCourseCard({ data }: { data: any }) {
  ...
  const getInitialBanner = () => {
    const first = media.find((m: any) => m.resource_type === 'image' || m.thumbnail_url);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6  px-4 pb-6">
      ...
      <motion.div
        ...
        className="bg-white max-w-xl ms-auto mx-auto lg:-mt-36 rounded-lg overflow-hidden shadow-lg border lg:w-1/3"
      >
        {/* Top Banner */}
        ...
        <div className="flex items-center justify-start space-x-3 p-4 overflow-x-auto">
          {media.map((item: any, i: number) => (
            <div
              ...
              onClick={() => setBanner(item.thumbnail_url || item.resource_value)}
            >
