...
export default function IeltsCourseCard() {
	...
	return (
		<div className="flex flex-col lg:flex-row gap-6  px-4 sm:px-6 lg:px-16 py-10">
			<div className="w-full mt-4 lg:w-2/3  bg-white">
				<Instructors
					instructors={course.data?.sections[2]?.values}
					heading={course.data?.sections[2]?.name}
				/>
				<CourseOverview
					cards={course.data?.sections[3]?.values}
					heading={course.data?.sections[3]?.name}
				/>
				<LearningOutcome
					outcomes={course.data?.sections[5]?.values}
					heading={course.data?.sections[5]?.name}
				/>
				<CourseContent />
				<CourseFeatures
					cousrseFeature={course.data?.sections[8]?.values}
					heading={course.data?.sections[8]?.name}
				/>
				<CourseDetails
					data={course.data?.sections[7]?.values}
					heading={course.data?.sections[7]?.name}
				/>
			</div>
			<div>
			<motion.div
				...
				className="bg-white w-full md:max-w-xl ms-auto h-screen mx-auto lg:-mt-36 rounded-lg overflow-hidden shadow-lg border lg:w-[30rem]"
			>
				{/* Top Banner */}
			...
			<CourseChecklist checklist={course?.data?.checklist} heading="Check List"/>
</div>
			{showStickyCard && (
